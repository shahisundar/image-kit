...
<script lang="ts">
  import { image, circle, rectangle, square, roundedRect  } from '@image-kit/core'
  ...
  async function loadSampleImage(url: string) {
    try {
      ...
      selectedImage = new File([blob], `sample-${Date.now()}.jpg`, { type: 'image/jpeg' });
      imageUrl = URL.createObjectURL(blob);
      
      processImage();
    } catch (err) {
      error = err instanceof Error ? err.message : 'Failed to load sample image';
    } finally {
      loading = false;
    }
  }
  ...
  // Update handlers
  function updateResize(event: CustomEvent<any>) {
    resizeOptions = event.detail;
    processImage();
  }

  function updateCrop(event: CustomEvent<any>) {
    cropShape = event.detail;
    processImage();
  }

  function updateQuality(event: CustomEvent<{quality: number, format: string, sharpeningLevel: number, algorithm: any}>) {
    quality = event.detail.quality;
    format = event.detail.format;
    sharpeningLevel = event.detail.sharpeningLevel;
  ...
  <div class="app-container">
      ...
      <div class="sample-images">
        ...
        <div class="samples">
          {#each sampleImages as sample}
            <button class="sample-button" on:click={() => loadSampleImage(sample.url)}>
              {sample.name}
            </button>
          {/each}
        </div>
    ...
    <div class="content-area">
      <div class="preview-section">
        <Preview originalUrl={imageUrl} processedUrl={processedImageUrl} loading={loading} />
      </div>
      ...
      <div class="controls-section">
        ...
        <div class="tab-content">
          {#if activeTab === 'resize'}
            <ImageResizer options={resizeOptions} on:update={updateResize} />
          {:else if activeTab === 'crop'}
            <ImageCropper shape={cropShape} on:update={updateCrop} />
          {:else if activeTab === 'quality'}
            <ImageQuality {quality} format={format} {sharpeningLevel} {algorithm} on:update={updateQuality} />
